...
2. Preflight the durable BigQuery destination before creating run-scoped GKE resources.
3. Provision the ephemeral GKE benchmark environment with Terraform.
4. Deploy the Online Boutique workload and monitoring stack with Helm.
5. Restart the load generator to start the measured benchmark window after monitoring is ready.
6. Observe the configured benchmark window.
7. Extract Prometheus metrics.
8. Generate and validate the benchmark summary.
9. Persist the validated benchmark summary to BigQuery.
10. Uninstall Helm releases and destroy the Terraform-managed GCP environment.
11. Capture traceability outputs.
12. Upload benchmark artifacts.
...
The service account must be able to create and tear down the ephemeral GKE resources and must have BigQuery permissions to inspect the summary table, query duplicate `run_id` values, create the preflight scratch table, load rows, and delete the scratch table. Manage the BigQuery writer path through `infra/terraform/gcp-bigquery` by passing the `GCP_SERVICE_ACCOUNT` email to `summary_writer_service_accounts`; the root grants `roles/bigquery.jobUser` on the project and `roles/bigquery.dataEditor` on the durable dataset.
...
The BigQuery preflight step validates the destination identifiers, table schema, query access, and a scratch-table load/delete before Terraform provisioning starts. If BigQuery is unavailable or the writer cannot create/load/delete tables, `bigquery-load-report.json` records the failed stage and bounded redacted stdout/stderr previews from the `bq` command.
...
GitHub Actions logs from May 11, 2026 warned that Node.js 20 actions will be forced to Node.js 24 on June 2, 2026 and removed from hosted runners on September 16, 2026. Keep the pinned workflow actions on Node.js 24-compatible releases before that transition.
