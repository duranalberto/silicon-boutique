...
- `run_benchmark_workflow.py`: recommended source-agnostic coordinator for local, GCP, AWS, and all-target benchmark workflows. It runs local automation directly, dispatches GCP/AWS through GitHub Actions, verifies downloaded artifacts, and generates dashboard files without serving them by default.
- `run_local_benchmark_workflow.py`: lower-level local workflow helper that verifies/repairs local Kubernetes, runs a smoke benchmark with BigQuery persistence, validates the remote row, and writes debug logs.
- `run_local_benchmark.py`: lower-level local/debug entrypoint for provision, deploy, benchmark, extract, summarize, validate, and cleanup without GitHub Actions.
